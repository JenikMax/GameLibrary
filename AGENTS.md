...
`docker-compose.yml` starts six services: `frontend` (Nginx + Vue SPA, `:8090`), `backend` (Spring Boot REST, `:8080`), `ai-service` (Python FastAPI, `:8000`), `transmission` (torrent seeder, `:9091` RPC, `:51413`), `postgresdb` (`:5432`), `emulatorjs` (self-hosted EmulatorJS WASM cores, no port, served via frontend at `/game-library/emu/`).  
Memory limits: postgresdb 512m, backend 1024m, ai-service 2048m, frontend 64m, transmission 256m, emulatorjs 256m. postgresdb has healthcheck.
...
- **Maket directory** (`maket/`). 28 standalone HTML-файлов — дизайн-прототипы для ретро-терминальной темы. Не являются частью приложения.
...
- **Browser emulation (EmulatorJS):**
  - **EmulationController** (`/api/games/{id}/emulation`, `/{id}/rom/**`, `/{id}/save-list`, `/{id}/save` GET/PUT). ROM-стриминг — **path-style** `GET /api/games/{id}/rom/**` (НЕ query-параметр: EmulatorJS резолвит .bin из .cue через `url.resolve(binName, gameUrl)`); поддержка HTTP Range (206/416, HEAD, Accept-Ranges) через `FileChannel.transferTo`. `getRequestURI()` не декодируется Tomcat → ручной `URLDecoder.decode` UTF-8 с защитой `"+"→"%2B"`. Path traversal защита: `resolve().normalize().startsWith(baseDir)`. `.bin` помечается `companion` только если упомянут в соседнем .cue (парсинг `FILE`-строк, ISO_8859_1, имена резолвятся относительно папки cue).
  - **EmulatorCatalog** — словарь платформ→ядро: ps1 (pcsx_rearmed / mednafen_psx_hw, ext {cue,bin,img,mdf,pbp,toc,cbn,ccd,chd}), psp (ppsspp, {iso,cso,pbp,elf}), nes/dendy (fceumm/nestopia), n64 (mupen64plus_next/parallel_n64), snes (snes9x). `resolve()` с алиасами (`dendy`, `playstation`, `sonyplaystation`, `supernintendo` и т.д.), `normalize()` — lowercase + стрип не-алфанумных.
  - **Сейвы** — серверные, per-user: таблица `emulator_save` (`14_emulator_save.sql`, UNIQUE(game_id,user_id,kind,slot)), сущность `EmulatorSave`, `EmulatorSaveService` (upsert, лимиты: srm ≤1МБ, state ≤32МБ). Аутентификация через httpOnly cookie — same-origin fetch из emulator.js сам её шлёт.
  - **emulatorjs/** — Docker-образ: alpine → wget с `https://cdn.emulatorjs.org/4.2.3/data/` (фиксировать стабильную версию! v4.3.0-pre — пререлиз), trim ядер (fceumm, nestopia, snes9x, mupen64plus_next, parallel_n64, pcsx_rearmed, mednafen_psx_hw, ppsspp), раздача через nginx:1.30-alpine. **`player.html`** — EJS-страница: выбор ядра (localStorage per system `gl-emu-core-prefs`, авто: deviceMemory≥8 && hardwareConcurrency≥4 → точное ядро, `.chd` → mednafen_psx_hw), синхронизация srm (30с + SHA-256-проверка изменений + beforeunload keepalive), state через `EJS_loadStateURL` + события.
  - **COOP/COEP** (`same-origin` + `require-corp`) — ТОЛЬКО на `location /game-library/emu/` в `frontend/nginx.conf` (не глобально, иначе YouTube-трейлеры сломаются). EJS без них не работает (SharedArrayBuffer для потоков).
  - **EmulatorJS API 4.2.3**: `EJS_player/EJS_core/EJS_gameUrl/EJS_gameID/EJS_pathtodata/EJS_threads/EJS_defaultOptions({"retroarch_core": core})/EJS_loadStateURL`; события `EJS_onGameStart/EJS_onSaveState(e.state)/EJS_onLoadState/EJS_onSaveSave(e.save)/EJS_onLoadSave/EJS_onExit` (return>0 отменяет дефолт); сейвы — `gameManager.getSaveFile()/loadSaveFiles()/getSaveFilePath()` + FS; ppsspp требует threads (WebGL2) и грузит `cores/ppsspp-assets.zip`.
  - **Frontend**: `api/emulation.js` (getEmulation/getSaveList/getPlayerUrl), кнопка «Играть» (severity danger, `v-if="emulation?.supported"`) в GameDetailView + Dialog выбора ROM при нескольких primary (поля бэкенда: `path, name, size, kind`). **PS2 — кнопка скрыта** (браузерных эмуляторов PS2 нет). Служебные папки `information/`, `img/` исключаются из сканирования ROM. Форматы `.img/.sub/.ccd/.mdf/.mds/.rar` не поддерживаются ядрами — игнорируются (конверсия в .chd/.pbp вручную).
  - **Миграция для существующих БД**: `docker compose exec postgresdb psql -U postgres -d game-library -f /docker-entrypoint-initdb.d/14_emulator_save.sql`.
